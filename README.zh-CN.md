...
1. 在 Microsoft Entra 中创建应用注册，并选择需要支持的账号类型。可以使用单租户应用；请从概述页复制其**目录（租户）ID**。
...
4. 将应用的**应用程序（客户端）ID**填入 Add2Calendar 设置。单租户应用还需要用其**目录（租户）ID**替换 `common`，然后点击**启用 Outlook 清理**。
...
| Microsoft 显示 AADSTS50194 | 应用是单租户，但颁发机构仍为 `common`。请在 **Microsoft 租户颁发机构**中填写目录（租户）ID，再次启用。 |
...
v0.4.1 已通过 **26 项单元测试和 15 项浏览器集成检查**。自动化测试不会写入真实学生日历。测试范围及仍需实测的项目见 [VALIDATION.md](VALIDATION.md)。
